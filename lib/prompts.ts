...
- NO MOVEMENT REUSE: a movement in the warm-up must NOT reappear as a working exercise — this includes lighter-dose or renamed versions of the same pattern (e.g., "walking lunges" in the warm-up AND as a working set is a violation; pick a different warm-up drill like leg swings or hip circles). No exercise may appear twice in "mainWorkout". Warm-ups prime with different, lighter drills; they are not working sets.
...
- SPLIT DAYS NEED THEIR COMPLEMENT: if the session trains only part of the body (upper-only, lower-only, push, pull, a single body part), "weeklySplitRecommendation" MUST outline the complementary day(s) so the client trains the whole body across the week (e.g., an upper day must pair with a lower/legs day). NEVER tell a client to run a single upper-only or lower-only session 2-3×/week as their entire program — that ships a legs-free or half-body plan.
- INTENSITY MUST MATCH THE GOAL: hypertrophy working sets must be taken close to failure — RPE 8-9 or "1-2 reps in reserve" on the main lifts (RPE 6-7 is too easy and becomes junk volume for muscle growth). Endurance/general-fitness can sit lower. Keep ONE RPE convention across the whole session (don't mix 7-8 and 7-9 for no reason).
- HYPERTROPHY PROGRESSION LEVERS: progress hypertrophy via harder variations, added load, more reps in range, added sets, or slower tempo/pauses — NOT by "reducing rest time" (shorter rest lowers per-set performance and is the wrong lever for growth). For bodyweight/limited-load hypertrophy, drive toward the harder variations, tempo, and pauses early, and be honest in "trainerNotes" that bodyweight-only muscle growth plateaus without added load — don't ship high-rep RPE 6-7 bodyweight work labelled as hypertrophy.
- LOWER-BODY & POSTERIOR CHAIN: a full-body session must include a hinge / posterior-chain movement (RDL, single-leg RDL, glute bridge, hip thrust) alongside a squat pattern — don't ship lower-body work that is squat/lunge only with no hinge. Balance lower-body volume, don't leave it to one or two sets.
- HYPERTROPHY DELT WORK: upper-body or full-body hypertrophy sessions should include at least one direct shoulder movement (overhead/lateral raise), not just incidental delt work from pressing.
